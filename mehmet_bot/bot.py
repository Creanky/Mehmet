...
async def on_message(message):
    ...
    # Sadece etiketlendiğinde yanıt ver
    if client.user not in message.mentions:
        return

    # Kanaldan son 15 mesajı çek (bağlam için)
    history = []
    ...
    # Kullanıcının mesajını al (etiket kısmını temizle)
...
Buna Mehmet olarak kısa ve komik bir cevap ver."""
    ...
    try:
        ...
        await message.reply("Abi bir şeyler ters gitti, sonra tekrar sor.")
        print(f"Hata: {e}")
